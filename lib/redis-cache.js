...
function RedisCache (options) {
  ...
  // Create promisified versions of Redis functions
  _.each(['get', 'expire', 'setex', 'del'], function (f) {
    if (!_.isFunction(self.client[f + 'Async'])) {
      self.client[f + 'Async'] = whenNode.lift(self.client[f].bind(self.client));
    }
  });
}
